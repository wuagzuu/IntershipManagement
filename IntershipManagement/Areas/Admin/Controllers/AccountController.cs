...
namespace IntershipManagement.Areas.Admin.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                // Đăng nhập tạm thời và chuyển hướng đến trang đổi mật khẩu
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (db.AspNetUsers.Any(c => c.Code == model.Code))
                {
                    ViewBag.role = context.Roles.ToList();
                    SetAlert("Tạo tài khoản không thành công. Mã định danh đã tồn tại", "danger");
                ...
                if (result.Succeeded)
                {
                    ...
                    if( _user != null)
                    {
                        ...
                        _user.Province = model.Province != null ? db.Provinces.Find(int.Parse(model.Province))?.Name : null;
                        _user.District = model.District != null ? db.Districts.Find(int.Parse(model.District))?.Name : null;
                        _user.Ward = model.Ward != null ? db.Wards.Find(int.Parse(model.Ward))?.Name : null;
                        if (model.Role == "Student")
                        {
                            var teacher = db.AspNetUsers.FirstOrDefault(c => c.UserName == User.Identity.Name);
                            _user.MajorId = teacher?.MajorId;
                            _user.IsStudent = true;
                        }
                        if (model.Role == "Teacher")
                        {
                            _user.MajorId = model.Major;
                            _user.IsTeacher = true;
                            var major = await db.Majors.FindAsync(model.Major);
                            if (major != null)
                            {
                                _user.Majors.Add(major);
                            }
                        }
                        _user.Code = model.Code;
