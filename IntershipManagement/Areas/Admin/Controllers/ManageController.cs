    ...
    public class ManageController : BaseController
    {
        ...
        public async Task<ActionResult> ChangePasswordFirstTime(ChangePasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                user.IsFirstLogin = false; // Đặt lại IsFirstLogin thành false
        ...
        public async Task<ActionResult> SetPassword(string Id)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.ResetPasswordAsync(Id, code, "Base@123456");
                if (result.Succeeded)
                {
                    SetAlert("Mật khẩu mặc định là Base@123456", "success");
                    return RedirectToAction("Index", "User");
                }
                AddErrors(result);
            }
            SetAlert("Reset mật khẩu không thành công", "danger");
            // If we got this far, something failed, redisplay form
            return RedirectToAction("Index", "User");
        }
